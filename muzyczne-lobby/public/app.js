...
function showAppForProfile() {
  ...
  roomCodeLabel.textContent = profile.role === "moderator" ? "GLOBALNY" : (profile.role === "solo" ? "SOLO" : profile.roomCode);
  if (roomContextLabel) roomContextLabel.textContent = profile.role === "moderator" ? "Panel" : "Pokoj";
  copyLinkButton.classList.toggle("hidden", profile.role === "solo" || profile.role === "moderator");
}
...
function setLoginMode(mode) {
  ...
  if (roomField) roomField.classList.toggle("hidden", isSoloLanding || isAdmin);
...
function connect() {
  ...
  socket.addEventListener("message", function (event) {
    ...
    if (payload.type === "joined") {
      ownId = payload.id;
      if (profile) {
        profile.roomCode = profile.role === "moderator" ? "GLOBAL" : payload.roomCode;
        if (profile.role === "moderator") profile.controlRoomCode = payload.controlRoomCode || "LOBBY";
      ...
      if (profile && profile.role === "moderator") url.searchParams.delete("room");
      else url.searchParams.set("room", payload.roomCode);
      history.replaceState(null, "", url);
    }
    if (payload.type === "adminRoomSelected") {
      if (profile && profile.role === "moderator") {
        profile.controlRoomCode = payload.roomCode || "LOBBY";
        saveSession();
        showToast("Sterowanie pokojem " + profile.controlRoomCode + ".");
      }
    }
...
function join(role) {
  ...
  const clientId = role === "solo" && savedSoloUser && savedSoloUser.clientId
    ...
    : makeClientId();
  const roomCode = role === "moderator" ? "GLOBAL" : (role === "solo" ? "SOLO" : cleanRoom(roomInput.value));
  ...
  roomCodeLabel.textContent = role === "moderator" ? "GLOBALNY" : roomCode;
  if (roomContextLabel) roomContextLabel.textContent = role === "moderator" ? "Panel" : "Pokoj";
  copyLinkButton.classList.toggle("hidden", role === "solo" || role === "moderator");
  connect();
  sendJoin();
}
...
function returnToLogin(message) {
  ...
  if (roomContextLabel) roomContextLabel.textContent = "Pokoj";
  resetTrackForm();
...
function primeSongAudioFromGesture() {
  unlockBuzzerSound();
  [audio, audioSecondary].forEach(function (media) {
    ...
    const restoreAudio = function () {
      if (restored) return;
      restored = true;
      if (!state || state.phase !== "playing") media.pause();
      media.muted = previousMuted;
      if (state && state.phase === "playing") syncAudio(false);
    };
...
function handlePreparedAudioEvent() {
  if (state && (state.phase === "loading" || state.phase === "countdown")) syncAudio(false);
}

[audio, audioSecondary].forEach(function (media) {
  media.addEventListener("loadeddata", handlePreparedAudioEvent);
  media.addEventListener("canplay", handlePreparedAudioEvent);
  media.addEventListener("seeked", handlePreparedAudioEvent);
...
if (adminPanelTabs) {
  function closeAdminNavDropdowns(exceptDropdown) {
    adminNavDropdowns.forEach(function (dropdown) {
      const isOpen = dropdown === exceptDropdown;
      dropdown.classList.toggle("open", isOpen);
      const trigger = dropdown.querySelector(".admin-nav-trigger");
  ...
  adminNavDropdowns.forEach(function (dropdown) {
    ...
    if (trigger) {
      trigger.addEventListener("click", function (event) {
        ...
        closeAdminNavDropdowns(dropdown.classList.contains("open") ? null : dropdown);
      });
    }

    dropdown.addEventListener("mouseenter", function () {
      if (!window.matchMedia("(hover: hover) and (pointer: fine)").matches) return;
      closeAdminNavDropdowns(dropdown);
    });
  ...
  document.addEventListener("click", function (event) {
    if (!event.target.closest(".admin-nav-dropdown")) closeAdminNavDropdowns();
  });
}
...
function trackSubmitLabel() {
  return editingTrackId ? "Zapisz w bibliotece" : "Dodaj do biblioteki";
}
...
trackForm.addEventListener("submit", function (event) {
  ...
  if (editingTrackId) {
    beginTrackSave("updateLibraryTrack", Object.assign({ trackId: editingTrackId }, payload));
  } else {
    mod("addLibraryTrack", payload);
    showToast("Dodano opening do biblioteki.");
    resetTrackForm();
  }
});
...
function render() {
  ...
  roomCodeLabel.textContent = profile && profile.role === "moderator" ? "GLOBALNY" : state.code;
  if (roomContextLabel) roomContextLabel.textContent = profile && profile.role === "moderator" ? "Panel" : "Pokoj";
  renderTrackHeader();
...
function openLibraryForTrack(reference) {
  if (!state || !reference) return;
  const referenceAnime = normalizeAnswerText(reference.anime || "");
  const referenceOpening = normalizeAnswerText(reference.opening || "");
  const referenceAudio = String(reference.audioUrl || "");
  const referenceVideo = String(reference.videoId || "");
  const track = (state.libraryTracks || []).find(function (candidate) {
    if (reference.trackId && candidate.id === reference.trackId) return true;
    if (referenceVideo && candidate.videoId === referenceVideo) return true;
    if (referenceAudio && candidate.audioUrl === referenceAudio) return true;
    return referenceAnime
      && normalizeAnswerText(candidate.anime || "") === referenceAnime
      && (!referenceOpening || normalizeAnswerText(candidate.opening || "") === referenceOpening);
  });

  librarySearch = (track && track.anime) || reference.anime || reference.opening || "";
  libraryDifficulty = "all";
  libraryPercent = "all";
  libraryStatus = "all";
  if (librarySearchInput) librarySearchInput.value = librarySearch;
  if (libraryDifficultyFilter) libraryDifficultyFilter.value = "all";
  if (libraryPercentFilter) libraryPercentFilter.value = "all";
  if (libraryStatusFilter) libraryStatusFilter.value = "all";
  setActiveAdminPanel("library", true);
  renderLibrary();
  showToast(track ? "Opening jest gotowy do edycji w bibliotece." : "Wyszukano opening w bibliotece.");
}
...
function renderAdminRooms() {
  ...
  rooms.forEach(function (room) {
    const row = node("div", "admin-room-row tournament-card");
    const meta = node("div", "track-meta");
...
function renderAdminQuality() {
  ...
  displayRows.forEach(function (entry) {
    ...
    const edit = node("button", "", "Biblioteka");
    save.type = "button";
    disable.type = "button";
    clear.type = "button";
    edit.type = "button";
    ...
    edit.addEventListener("click", function () { openLibraryForTrack(entry); });
...
function renderAdminSoloStats() {
  ...
  categories.forEach(function (category) {
    ...
    rows.forEach(function (entry) {
      ...
      const openLibrary = node("button", "", "Biblioteka");
      ...
      openLibrary.addEventListener("click", function () { openLibraryForTrack(entry); });
      difficultyTools.append(currentDifficulty, qualityPill(entry.qualityStatus || "ok", entry.disabled), qualityInput, changeQuality, openLibrary);
...
function renderAdminReports() {
  ...
  visibleReports.forEach(function (report) {
    ...
    const edit = node("button", "", "Pokaż w bibliotece");
    const remove = node("button", "danger-button", "Usun");
    edit.type = "button";
    remove.type = "button";
    edit.addEventListener("click", function () { openLibraryForTrack(report); });
    remove.addEventListener("click", function () {
...
function editTrack(track, source) {
  ...
  trackSubmitButton.textContent = "Zapisz w bibliotece";
  trackSubmitButton.disabled = false;
  cancelEditButton.disabled = false;
...
function resetTrackForm() {
  ...
  trackSubmitButton.textContent = "Dodaj do biblioteki";
  trackSubmitButton.disabled = false;
  cancelEditButton.disabled = false;
