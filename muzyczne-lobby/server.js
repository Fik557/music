...
function buildGlobalCatalogConfig() {
  const trackMap = new Map();
  const libraryMap = new Map();
  let selectedSettings = { difficultyScores: cloneScores(DEFAULT_DIFFICULTY_SCORES) };

  Object.keys(persistedRoomConfigs)
    .filter(function (code) {
      const config = persistedRoomConfigs[code];
      return !isMetaConfigKey(code) && config && typeof config === "object";
    })
    .sort(function (left, right) {
      return numberInRange(persistedRoomConfigs[left] && persistedRoomConfigs[left].updatedAt, 0, 0, 9999999999)
        - numberInRange(persistedRoomConfigs[right] && persistedRoomConfigs[right].updatedAt, 0, 0, 9999999999);
    })
    .forEach(function (code) {
      const config = persistedRoomConfigs[code] || {};
      restoreTracks(config.tracks).forEach(function (track) {
        track.result = null;
        trackMap.set(soloTrackKey(track), track);
      });
      restoreTracks(config.libraryTracks).forEach(function (track) {
        track.result = null;
        libraryMap.set(soloTrackKey(track), track);
      });
      if (config.settings && config.settings.difficultyScores) {
        selectedSettings = { difficultyScores: cloneScores(config.settings.difficultyScores) };
      }
    });

  adminAuditStore()
    .filter(function (entry) { return entry && entry.status === "success" && entry.after; })
    .sort(function (left, right) {
      return numberInRange(left.at, 0, 0, 9999999999) - numberInRange(right.at, 0, 0, 9999999999);
    })
    .forEach(function (entry) {
      const edited = restoreTracks([entry.after])[0];
      if (!edited) return;
      edited.result = null;
      const key = soloTrackKey(edited);
      if (entry.action === "updateTrack") trackMap.set(key, edited);
      if (entry.action === "updateLibraryTrack") libraryMap.set(key, edited);
      if (entry.action === "updateReportedTrack") {
        if (trackMap.has(key)) trackMap.set(key, edited);
        if (libraryMap.has(key)) libraryMap.set(key, edited);
      }
    });

  if (!libraryMap.size) {
    trackMap.forEach(function (track, key) {
      libraryMap.set(key, Object.assign({}, track, { result: null }));
    });
  }

  return {
    tracks: Array.from(trackMap.values()),
    libraryTracks: Array.from(libraryMap.values()),
    settings: selectedSettings,
    updatedAt: now()
  };
}

function ensureGlobalCatalogConfig() {
  const existing = persistedRoomConfigs.__globalCatalog;
  if (existing && typeof existing === "object" && Array.isArray(existing.tracks) && Array.isArray(existing.libraryTracks)) {
    return existing;
  }
  persistedRoomConfigs.__globalCatalog = buildGlobalCatalogConfig();
  savePersistedRooms();
  return persistedRoomConfigs.__globalCatalog;
}

function getGlobalCatalogRoom() {
  if (globalCatalogRoom) return globalCatalogRoom;
  const config = ensureGlobalCatalogConfig();
  globalCatalogRoom = {
    code: "GLOBAL",
    tracks: restoreTracks(config.tracks),
    libraryTracks: restoreTracks(config.libraryTracks),
    currentTrackId: null,
    settings: {
      clipDuration: DEFAULT_CLIP_DURATION,
      segmentSplit: 5,
      difficultyScores: cloneScores(config.settings && config.settings.difficultyScores)
    },
    updatedAt: numberInRange(config.updatedAt, now(), 0, 9999999999)
  };
  return globalCatalogRoom;
}

function catalogTracksForRoom(existingTracks, catalogTracks) {
  const resultsById = new Map();
  const resultsByKey = new Map();
  restoreTracks(existingTracks).forEach(function (track) {
    if (!track.result) return;
    if (track.id) resultsById.set(track.id, track.result);
    resultsByKey.set(soloTrackKey(track), track.result);
  });
  return restoreTracks(catalogTracks).map(function (track) {
    track.result = resultsById.get(track.id) || resultsByKey.get(soloTrackKey(track)) || null;
    return track;
  });
}

function syncGlobalCatalogToRooms(catalog) {
  const activeCodes = new Set();

  rooms.forEach(function (room, code) {
    activeCodes.add(code);
    const previousCurrent = room.tracks.find(function (track) { return track.id === room.currentTrackId; });
    const previousId = previousCurrent ? previousCurrent.id : "";
    const previousKey = previousCurrent ? soloTrackKey(previousCurrent) : "";
    room.tracks = catalogTracksForRoom(room.tracks, catalog.tracks);
    room.libraryTracks = restoreTracks(catalog.libraryTracks);
    const selected = room.tracks.find(function (track) { return previousId && track.id === previousId; })
      || room.tracks.find(function (track) { return previousKey && soloTrackKey(track) === previousKey; });
    room.currentTrackId = selected ? selected.id : (room.tracks[0] ? room.tracks[0].id : null);
    room.settings.difficultyScores = cloneScores(catalog.settings.difficultyScores);
    touch(room);
    persistedRoomConfigs[code] = {
      tracks: room.tracks,
      libraryTracks: room.libraryTracks,
      currentTrackId: room.currentTrackId,
      blockedIps: room.blockedIps,
      updatedAt: room.updatedAt,
      settings: { difficultyScores: cloneScores(room.settings.difficultyScores) }
    };
    broadcast(room);
  });

  Object.keys(persistedRoomConfigs).forEach(function (code) {
    if (isMetaConfigKey(code) || activeCodes.has(code)) return;
    const config = persistedRoomConfigs[code];
    if (!config || typeof config !== "object") return;
    const previousTracks = restoreTracks(config.tracks);
    const previousCurrent = previousTracks.find(function (track) { return track.id === config.currentTrackId; });
    const previousId = previousCurrent ? previousCurrent.id : "";
    const previousKey = previousCurrent ? soloTrackKey(previousCurrent) : "";
    config.tracks = catalogTracksForRoom(previousTracks, catalog.tracks);
    config.libraryTracks = restoreTracks(catalog.libraryTracks);
    const selected = config.tracks.find(function (track) { return previousId && track.id === previousId; })
      || config.tracks.find(function (track) { return previousKey && soloTrackKey(track) === previousKey; });
    config.currentTrackId = selected ? selected.id : (config.tracks[0] ? config.tracks[0].id : null);
    config.settings = { difficultyScores: cloneScores(catalog.settings.difficultyScores) };
    config.updatedAt = now();
  });
}

function saveGlobalCatalog(catalog) {
  catalog.updatedAt = now();
  persistedRoomConfigs.__globalCatalog = {
    tracks: catalog.tracks,
    libraryTracks: catalog.libraryTracks,
    settings: { difficultyScores: cloneScores(catalog.settings.difficultyScores) },
    updatedAt: catalog.updatedAt
  };
  syncGlobalCatalogToRooms(catalog);
...
function createRoom(code) {
  ...
  const base = ensureGlobalCatalogConfig();
  ...
  const sourceTracks = baseHasTracks ? base.tracks : (savedHasTracks ? saved.tracks : []);
  const sourceLibraryTracks = Array.isArray(base.libraryTracks) && base.libraryTracks.length
    ? base.libraryTracks
    : (Array.isArray(saved.libraryTracks) && saved.libraryTracks.length ? saved.libraryTracks : sourceTracks);
  const sourceSettings = base.settings || saved.settings || {};
  const savedCurrentTrack = restoreTracks(saved.tracks).find(function (track) {
    return track.id === saved.currentTrackId;
  });
  const savedCurrentKey = savedCurrentTrack ? soloTrackKey(savedCurrentTrack) : "";
  ...
  const selectedTrack = tracks.find(function (track) {
    return savedCurrentKey && soloTrackKey(track) === savedCurrentKey;
  });
  const selectedTrackId = selectedTrack ? selectedTrack.id : (tracks[0] ? tracks[0].id : null);
...
function isMetaConfigKey(code) {
  return code === "__soloStats" || code === "__soloReports" || code === "__soloPlayers" || code === "__dailySolo" || code === "__soloDayChampion" || code === "__adminAudit" || code === "__globalCatalog";
}
...
function recordAdminEdit(socket, room, action, status, message, before, after, payload, repeatedFrom) {
  ...
  const entry = {
    ...
    repeatable: action === "updateLibraryTrack",
...
function allSoloTracks() {
  const pool = [];
  const seen = {};
  const catalog = getGlobalCatalogRoom();
  (catalog.libraryTracks || []).forEach((track) => addSoloTrack(pool, seen, track));
...
function publicRoom(room, socket) {
  ...
  const catalog = isModerator ? getGlobalCatalogRoom() : null;
  const roundClosed = isRoundClosed(room);
  const scope = "room:" + room.code + ":" + socket.role + (isModerator ? ":global" : "");
  const roomState = {
    code: room.code,
    phase: room.phase,
  ...
  addChangedStateField(roomState, socket, scope, "tracks", isModerator ? catalog.tracks : []);
  addChangedStateField(roomState, socket, scope, "libraryTracks", isModerator ? catalog.libraryTracks : []);
  ...
  addChangedStateField(roomState, socket, scope, "settings", isModerator ? Object.assign({}, room.settings, {
    difficultyScores: cloneScores(catalog.settings.difficultyScores)
  }) : room.settings);
...
function broadcastModeratorStats() {
  sockets.forEach(function (socket) {
    if (socket.role === "moderator" && socket.joined) {
      send(socket, publicRoom(getRoom(socket.adminRoomCode || "LOBBY"), socket));
    }
  });
}
...
function joinRoom(socket, payload) {
  ...
  const room = requestedRole === "moderator" ? getRoom("LOBBY") : getRoom(payload.roomCode);
  const previous = socket.roomCode ? getRoom(socket.roomCode) : null;
  ...
  socket.joined = true;
  socket.roomCode = socket.role === "moderator" ? "" : room.code;
  socket.adminRoomCode = socket.role === "moderator" ? "LOBBY" : "";

  send(socket, {
    type: "joined",
    id: socket.id,
    roomCode: socket.role === "moderator" ? "GLOBAL" : room.code,
    controlRoomCode: socket.role === "moderator" ? room.code : "",
    role: socket.role,
    team: socket.team,
    playMode: socket.playMode || "group"
  });
...
function isGlobalCatalogAction(action) {
  return [
    "addTrack",
    "addLibraryTrack",
    "addLibraryToMain",
...
function updateSoloStatDifficulty(payload) {
  ...
  const catalog = getGlobalCatalogRoom();
  let changed = updateTrackListDifficultyByKey(catalog.tracks, statKey, difficulty)
    + updateTrackListDifficultyByKey(catalog.libraryTracks, statKey, difficulty);
  ...
  touch(catalog);
  const persistenceError = saveGlobalCatalog(catalog);
...
function updateReportedTrack(payload) {
  ...
  try {
    const tracksResult = updateTrackListByReportKey(catalog.tracks, statKey, payload);
    const libraryResult = updateTrackListByReportKey(catalog.libraryTracks, statKey, payload);
    changed = tracksResult.changed + libraryResult.changed;
    sampleTrack = tracksResult.sampleTrack || libraryResult.sampleTrack;
  } catch (error) {
...
function removeRoom(socket, payload) {
  ...
  const deletingCurrent = socket.adminRoomCode === targetCode;
  ...
  if (deletingCurrent) {
    ...
    socket.adminRoomCode = fresh.code;
    socket.stateFieldSignatures = Object.create(null);
    send(socket, { type: "adminRoomSelected", roomCode: fresh.code });
    send(socket, publicRoom(fresh, socket));
  }
...
function repeatAdminEdit(socket, currentRoom, payload) {
  ...
  const targetRoom = getGlobalCatalogRoom();
  const before = findTrackForAdminEdit(targetRoom, action, editPayload);
  let error = "";

  error = updateLibraryTrack(targetRoom, editPayload) || "";

  if (!error) {
    touch(targetRoom);
    error = saveGlobalCatalog(targetRoom) || "";
    broadcastModeratorStats();
  }
...
async function handleModerator(socket, payload) {
  if (!socket.joined || !requireModerator(socket)) return;
  const action = payload.action;
  if (["updateTrack", "updateReportedTrack", "updateSoloStatDifficulty"].includes(action)) {
    return sendError(socket, "Dane openingu mozna edytowac tylko w bibliotece.", payload.requestId);
  }
  if (action === "selectAdminRoom") {
    const targetRoom = getRoom(payload.roomCode || payload.code);
    socket.adminRoomCode = targetRoom.code;
    socket.stateFieldSignatures = Object.create(null);
    send(socket, { type: "adminRoomSelected", roomCode: targetRoom.code });
    send(socket, publicRoom(targetRoom, socket));
    return;
  }

  const controlRoom = getRoom(socket.adminRoomCode || "LOBBY");
  const globalCatalogAction = isGlobalCatalogAction(action);
  const room = globalCatalogAction ? getGlobalCatalogRoom() : controlRoom;
