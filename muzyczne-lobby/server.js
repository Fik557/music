...
function restoreTracks(tracks) {
  ...
  return tracks
    ...
    .map(function (track) {
      ...
      const anime = rawText(track.anime || track.title, 90) || "Anime bez nazwy";
      const opening = rawText(track.opening || track.artist, 90);
...
function createRoom(code) {
  ...
  const tracks = restoreTracks(saved.tracks);
  const selectedTrackId = tracks.some(function (track) {
    return track.id === saved.currentTrackId;
  }) ? saved.currentTrackId : (tracks[0] ? tracks[0].id : null);
  ...
  return {
    ...
    settings: {
      clipDuration: 10,
      segmentSplit: 5,
      difficultyScores: cloneScores(saved.settings && saved.settings.difficultyScores)
    },
    updatedAt: now()
  };
}
...
function collectPlaylistVideos(node, videos, seen) {
  ...
  Object.keys(node).forEach(function (key) {
    collectPlaylistVideos(node[key], videos, seen);
  });
}
...
function extractPlaylistVideosFromHtml(html) {
  ...
  if (initialData) {
    try {
      collectPlaylistVideos(JSON.parse(initialData), videos, seen);
    } catch (error) {
      videos.length = 0;
  ...
  for (const match of matches) {
    ...
    if (!seen[videoId]) {
      seen[videoId] = true;
      videos.push({ videoId: videoId, title: "YouTube opening " + (videos.length + 1) });
...
async function importPlaylistTracks(room, payload) {
  ...
  if (!room.currentTrackId && room.tracks[0]) room.currentTrackId = room.tracks[0].id;
  return { added: added };
}
...
function normalizeTrack(payload, existing) {
  ...
  const anime = rawText(payload.anime || payload.title, 90) || (source === "youtube" ? "Anime z YouTube" : "Anime bez nazwy");
  const opening = rawText(payload.opening || payload.artist, 90);
...
function peopleForRoom(room, isModerator) {
  ...
  clients.forEach(function (client) {
    ...
    if (maxByTeam[client.team] == null || score > maxByTeam[client.team]) maxByTeam[client.team] = score;
  });

  return clients
    .map(function (client) {
      const personalScore = Number(client.personalScore || 0);
      const mvpScore = maxByTeam[client.team] || 0;
      return {
        id: client.id,
    ...
    .sort(function (a, b) {
      return Number(b.role === "moderator") - Number(a.role === "moderator") || a.team.localeCompare(b.team) || b.personalScore - a.personalScore || a.nickname.localeCompare(b.nickname);
    });
}
...
function joinRoom(socket, payload) {
  ...
  send(socket, { type: "joined", id: socket.id, roomCode: room.code, role: socket.role });
...
async function handleModerator(socket, payload) {
  ...
  if (action === "importPlaylist") {
    ...
    send(socket, { type: "playlistImported", message: "Dodano openingi z playlisty: " + result.added + "." });
  }
  ...
  } else if (["addTrack", "importPlaylist", "updateTrack", "removeTrack", "selectTrack", "updateSettings", "play", "guessed", "revealTitle", "blockIp", "unblockIp"].includes(action)) {
    saveRoomConfig(room);
  }
